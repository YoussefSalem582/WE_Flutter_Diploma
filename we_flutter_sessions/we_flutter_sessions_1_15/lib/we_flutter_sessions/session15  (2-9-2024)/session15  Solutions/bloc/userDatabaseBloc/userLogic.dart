...
class UserLogic extends Cubit<UserState> {
  ...
  createDatabaseAndTable() async {
    await openDatabase("u.db", version: 3, onCreate: (Database d, int i) async {
  ...
  insertDepartment(
      {required int id, required String name, required int manager_id}) async {
    await db.transaction((txn) async {
      txn
          .rawInsert("insert into department "
  ...
  insertEmployee(
      ...
      required String name,
      required int salary,
      required int de_id}) async {
    await db.transaction((txn) async {
      txn
          .rawInsert("insert into employee "
